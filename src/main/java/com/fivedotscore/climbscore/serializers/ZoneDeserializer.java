...
public class ZoneDeserializer extends JsonDeserializer<Zone> {
    ...
    Logger logger = LoggerFactory.getLogger(ZoneDeserializer.class);

    @Override
    public Zone deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {

        JsonNode node = jsonParser.getCodec().readTree(jsonParser);

        Long id = node.asLong();

        return roundService.findZoneById(id);
    }
}
